...
export default function DeadlineAlerts({ jobs }) {
  ...
  const upcoming = jobs
    ...
    .map((job) => ({
      ...job,
      daysLeft: differenceInCalendarDays(new Date(job.deadline), today),
    }))
  ...
  return (
    <div className="bg-amber-500/15 border border-amber-500/40 rounded-xl p-4 mb-6 flex items-start justify-between gap-4">
        ...
        <ul className="flex flex-col gap-1">
          {upcoming.map((job) => (
            <li
              ...
              className="text-sm text-amber-200 cursor-pointer hover:underline"
            >
